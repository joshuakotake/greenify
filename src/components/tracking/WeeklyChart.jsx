...
const WeeklyCO2Chart = ({ dailyBudget = 20 }) => {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">
            Usage: <span className="font-medium">{data.usage.toFixed(1)} kg CO2</span>
          </p>
          <p className="text-sm text-gray-600">
  ...
  const daysOverBudget = weeklyData.filter(day => day.usage > day.budget).length;
    ...
    <div className="w-full max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="text-sm text-gray-600 font-medium">Weekly Budget</div>
            <div className="text-2xl font-bold text-gray-900">
              {totalBudget} kg CO2
            </div>
          </div>
          <div className={`p-4 rounded-lg ${daysOverBudget > 0 ? 'bg-red-50' : 'bg-green-50'}`}>
            <div className={`text-sm font-medium ${daysOverBudget > 0 ? 'text-red-600' : 'text-green-600'}`}>
              Days Over Budget
            </div>
            <div className={`text-2xl font-bold ${daysOverBudget > 0 ? 'text-red-900' : 'text-green-900'}`}>
              {daysOverBudget}/7
            </div>
          </div>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={weeklyData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            <YAxis 
              ...
              tickLine={false}
              tick={{ fontSize: 12, fill: '#6b7280' }}
              label={{ value: 'CO2 Usage (kg)', angle: -90, position: 'insideLeft' }}
            />
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <div className="w-4 h-1 bg-red-400 rounded" style={{backgroundImage: 'repeating-linear-gradient(to right, #ef4444 0, #ef4444 4px, transparent 4px, transparent 8px)'}}></div>
        ...
        <div className="text-sm text-gray-600">
          Weekly Progress: 
          <span className={`ml-1 font-medium ${weeklyProgress > 100 ? 'text-red-600' : 'text-green-600'}`}>
